...
public class Laberinto {
    ...
    private void escribirTechTalents() {
    	int[][] techTalents=TechTalents.getArray();
    	for(int i=0; i<ancho; i++){
            for(int j=0; j<techTalents[i].length ;j++){
            	if(j<alto){
	                if(ladrillos[i][j]){
	                	if(techTalents[i][j]==1){
    ...
    private void annadir(int i, int j) {
    	double distanciaAlCentro = Math.sqrt((i-ancho/2)*(i-ancho/2)+(j-alto/2)*(j-alto/2));
